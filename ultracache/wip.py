...
def cache_meta(request, lcache_key, lstart_index=0):
    ...
    print "CACHE_META"
    #for start_index, cache_key in reversed(getattr(request, "_ultracache_points", [(lstart_index, lcache_key)])):
    for start_index, end_index, cache_key in reversed(getattr(request, "_ultracache_points")):
        ...
        #print "TO SET PATHS"
        #print to_set_paths
        #print "TO SET CONTENT TYPES"
        #print to_set_content_types
        #print "TO SET CONTENT TYPES PATHS"
        #print to_set_content_types_paths
        #print "TO SET OBJECTS"
        #print to_set_objects

        # Deletion must happen first because set may set some of these keys
        if to_delete:
            if "template.cache.'test_ultracache_invalidate_two'.c4ca4238a0b923820dcc509a6f75849b" in to_delete:
        ...
        if di:
            if "template.cache.'test_ultracache_invalidate_two'.c4ca4238a0b923820dcc509a6f75849b" in di:
                import pdb;pdb.set_trace()
            try:
